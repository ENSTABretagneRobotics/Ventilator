...
while (not bExit):
    ...
    if (flow_control < flow_control_max):
        if (((valve_air_val > 0) and (valve_O2_val > 0)) and ((mode == 2) or (t-t_cycle_start < inspi_duration))): # Proportional valve control
            flow_control_pressure_excess = max(0, min(flow_control, flow_control_pressure_excess-coef_pressure_excess*(float(flow_control)/float(flow_control_max))*dt*err_pressure_excess)) # To limit flow if pressure is too high...

            err_flow_air = flow_control_pressure_excess*max(0.0, 1.0-O2_percent*0.01)-flow_filtered_air*60000.0 # In L/min
            if ((err_flow_air < 0) or ((t-t_valve_air_closed > valves_delay) and (0.01*ramp*inspi_duration <= 0)) or ((0.01*ramp*inspi_duration > 0) and (t-t_cycle_start > 0.01*ramp*inspi_duration))): # Avoid accumulating error when flow is low just due to valve delay or ramp at the beginning of inspiration...
            ...
            err_flow_O2 = flow_control_pressure_excess*O2_percent*0.01-flow_filtered_O2*60000.0 # In L/min
            if ((err_flow_O2 < 0) or ((t-t_valve_O2_closed > valves_delay) and (0.01*ramp*inspi_duration <= 0)) or ((0.01*ramp*inspi_duration > 0) and (t-t_cycle_start > 0.01*ramp*inspi_duration))): # Avoid accumulating error when flow is low just due to valve delay or ramp at the beginning of inspiration...
